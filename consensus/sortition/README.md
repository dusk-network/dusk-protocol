...
$$ M^C = (pk, credits),$$ where $pk$ is the public key of the provisioner, and $credits$ is the number of voting credits assigned to such provisioner by the $DS$ algorithm.
...
- $C[i] = M_i^C$ denotes the $i$th member of $C$
- $C[pk] =  M_i^C : M_i^C.pk = pk$ (or $nil$ if $pk$ is not the committee)

### Reduction Committees
Voting Committees in the Reduction steps have a credit pool of size $VCPool$, which is a global consensus parameter, and is currently set to $64$.

During a Reduction step, votes from a given member are multiplied by its influence in the committee. For instance, if a member has 3 credits, his vote will be counted 3 times.
...
To extract a block generator, a one-member committee is created, using the $Deterministic Sortition$ procedure with $PS = 1$, that is, by assigning a single credit. The provisioner being assigned such a credit becomes the block generator for the iteration.
...
## Algorithm
The *Deterministic Sortition* algorithm generates a voting committee by assigning the credits of credit pool to eligible provisioners (see [Eligibility](../README.md#participants)).

The algorithm uses the provisioner set $\mathcal{P}$, the block produced in the previous round $B_{R-1}$, and the $CreditPoolSize$.

It takes as inputs the round number $R$ and step number $S$, and outputs the corresponding committee.


The algorithm assigns one credit at a time to a randomly-selected provisioner using the *Deterministic Extraction* protocol, which is based on a pseudo-random number called [*score*](#score), described below.
...
To balance out this probability each Provisioner weight is initially set to the value of its stake, and then reduced by 1 DUSK each time the Provisioner is assigned a credit (if the weight is lower than 1 DUSK, it is set to 0).
This way, the probability for a provisioner to be extracted diminishes with every assigned credit.

As a result, on average, eligible Provisioners will participate in committees with a frequency and influence proportional to their stakes.
...
### Seed
<!-- TODO: Seed_{B_0} = crypto.RandEntropy(33) -->
The *Seed* value is used to ensure the pseudo-randomicity and unpredictability of the score.

This value is included in the block header (see [Block Structure](../../blockchain/README.md#header-structure)) and gets updated for each block. 
...
Note that the $Seed_0$ is a randomly generated value contained in the genesis block $B_0$.
...
In the following, we describe both procedures.
...
*Parameters*:
 ...
 - $S$: consensus step
 - $PS$: credit pool size
 - $W_P$: stake *weight* of provisioner $P$
...
$DS(R, S, PS)$:
- $ W_P = P.Stake, \forall P \in \mathcal{P}^R $
...
*Algorithm*:
  - For each provisioner $P$ in $\mathcal{P}^O$
    - if $P$'s weight is higher than $Score$, output $P$
    - otherwise, subtract $P$'s weight from $Score$
